...
class ChampionshipController extends AbstractController
{
    ...
    public function index(
        ...
        return $this->render("championship/index.html.twig", [
            "championships" => array_filter(
                $championshipRepository->findAll(),
                fn(Championship $championship) => $this->isGranted(
    ...
    public function show(
        ...
        $allChampionshipPlayers = $playerRepository->getAllPlayerByChampionShip(
            $championship
        );
        $allTournaments = $tournamentRepository->findBy([
            "championship" => $championship,
        ]);
        usort(
            $allChampionshipPlayers,
            fn(
                Player $playerA,
                Player $playerB
            ) => $playerB->getTotalPointsByChampionship($championship) <=>
                $playerA->getTotalPointsByChampionship($championship)
        );
    ...
    public function edit(
        ...
        if (!$this->isGranted("EDIT_CHAMPIONSHIP_TOURNAMENT", $championship)) {
            $this->addFlash(
                "error",
                "Vous n'avez pas les droits pour effectuer cette action"
            );
    ...
    public function delete(
        ...
        if (
            !$this->isGranted("DELETE_CHAMPIONSHIP_TOURNAMENT", $championship)
        ) {
            $this->addFlash(
                "error",
                "Vous n'avez pas les droits pour effectuer cette action"
