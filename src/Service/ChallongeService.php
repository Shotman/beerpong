...
class ChallongeService
{
    ...
    public function createTournament(?Tournament $tournament, array $teams)
    {
        ...
        if(is_null($tournament->getChallongeId())){
            $createTournamentResponse = $this->client->request("POST",$this->baseUrl."/tournaments.json",["query" => ["api_key" => $this->parameterBag->get("challonge_api_key"),"tournament" => [
                "name" => $tournament->getName(),
                "open_signup" => false,
                "private" => true,
                "game_name" => $tournament->getExtraData()["game"] ?? "BeerPong",
            ]]]);
    ...
    public function getTournamentDetails($tournament, $refresh = false): array
    {
        ...
        if(!is_null($tournamentDb) && $tournamentDb->getTournamentResults()->count() > 0){
            return [
               ...
               "number_of_teams" => $tournamentDb->getExtraData()["number_of_teams"],
               "participants" => $tournamentDb->getTournamentResults()->map(function ($tournamentResult){
                   return [
    ...
    private function addParticipantsToTournament($tournamentChallonge, array $teams): void
    {
        ...
        try {
            $this->client->request("POST",$this->baseUrl."/tournaments/".$tournamentChallonge->getChallongeId()."/participants/bulk_add.json",
                [
                    "query" => $query,
                    "body" => $participants,
                    "headers" => [
    ...
    private function randomizeTournament($tournamentChallonge)
    {
        ...
        $this->client->request("POST",$this->baseUrl."/tournaments/".$tournamentChallonge->getChallongeId()."/participants/randomize.json",
            [
                "query" => $query,
            ]
        );
    ...
    public function startTournament($tournamentChallonge): mixed
    {
        ...
        return $this->client->request("POST",$this->baseUrl."/tournaments/".$tournamentChallonge->getChallongeId()."/start.json",
            [
                "query" => $query,
            ]
        )->getContent();
    ...
    public function deleteTournament( Tournament $tournament): void
    {
        ...
        $this->client->request("DELETE",$this->baseUrl."/tournaments/".$tournament->getChallongeId().".json",
            [
                "query" => $query,
            ]
        );
